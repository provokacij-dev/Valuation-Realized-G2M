...
interface SkillsManagerProps {
  /** When embedded under another heading, render as h2 instead of h1 */
  embedded?: boolean;
  /** Hide the "Skills" title row (e.g. when the parent already shows one) */
  hideTitle?: boolean;
}

export default function SkillsManager({ embedded = false, hideTitle = false }: SkillsManagerProps) {
  ...
  const titleClass = embedded
    ? "text-lg font-semibold text-vr-green"
    : "text-2xl font-bold text-vr-green";

  return (
    <div>
      <div className={`flex items-center ${hideTitle ? "justify-end" : "justify-between"} mb-6`}>
        {!hideTitle && <TitleTag className={titleClass}>Skills</TitleTag>}
        <button
